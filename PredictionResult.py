...
def prediction(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], pvalue = 0.05):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=5) # 70% training and 30% test
    ...
    testingData = pd.DataFrame(columns=['y_test','y_pred'])
...
#one column only
def prediction1(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], pvalue = 0.05):
    if len(lectureList) == 0:
        lectureList = dataProcessing.getLectureList(dfActivityMatrixWithResult)
    ...
    # ex1_LogPageIf = dataProcessing.addCompleteTimeToEventLog(ex1_LogPageIf)
    # LogPageActivityCountByUser = logPage.groupby([pd.Grouper(key='org:resource'),pd.Grouper(key='concept:name')]).count()
    LogPageactivityCountByUser = FCAMiner.transitionDataMatrixConstruct_for_prediction(logPage)
    LogPageactivityCountByUser = LogPageactivityCountByUser.fillna(0)
    ...
    colSelection = [] #select only col whose p-value <=0.05 with result exam
    for col in LogPageactivityCountByUser.columns:
         if col != 'result_exam_1' and col != 'case':
        #     corr = pearsonr(LogPageactivityCountByUser['result_exam_1'], LogPageactivityCountByUser[col])
        #     if corr[1] <= pvalue:
        #         colSelection.append(col)    
            colSelection.append(col)
...
def predictionAdaBoost(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], pvalue = 0.05):
    if len(lectureList) == 0:
        lectureList = dataProcessing.getLectureList(dfActivityMatrixWithResult)
    ...
    testingData['y_test'] = y_test
    testingData['y_pred'] = y_pred
    return [metricsResult, feature_imp, classifier,LogPageactivityCountByUser,testingData]

def predictionGradientBoostClassification(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], pvalue = 0.05):
    if len(lectureList) == 0:
        lectureList = dataProcessing.getLectureList(dfActivityMatrixWithResult)
...
def XGBoostClassification(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], pvalue = 0.05):
    if len(lectureList) == 0:
        lectureList = dataProcessing.getLectureList(dfActivityMatrixWithResult)
    ...
    weak_LogPageActivityCountByUser['result_exam_1'] = 0
    LogPageactivityCountByUser = pd.concat([excellent_LogPageActivityCountByUser,weak_LogPageActivityCountByUser])
    # cum_practice_col = ['cumm_practice'] #,'correct_adjusted,cumm_practice','successPassedRate'
    # LogPageactivityCountByUser = LogPageactivityCountByUser.merge(practice.loc[:,cum_practice_col], 
    #                                         left_on=LogPageactivityCountByUser.index, right_on=practice.index)    
    # LogPageactivityCountByUser = LogPageactivityCountByUser.set_index('key_0')
...
def XGBoostClassificationProba(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], pvalue = 0.05):
    if len(lectureList) == 0:
        lectureList = dataProcessing.getLectureList(dfActivityMatrixWithResult)
...
def predict_proba_all_algorithms(dfActivityMatrixWithResult, excellentList, weakList,practice, lectureList=[], mode = 'activity', originalElements = []):
    if len(lectureList) == 0:
        lectureList = dataProcessing.getLectureList(dfActivityMatrixWithResult)
    ...
    if mode == 'activity':
        ...
        LogPageactivityCountByUser = FCAMiner.transitionDataMatrixConstruct_for_prediction(logPage,originalElements)
        LogPageactivityCountByUser = LogPageactivityCountByUser.fillna(0)
        LogPageactivityCountByUser = LogPageactivityCountByUser.groupby([pd.Grouper(key='user')]).sum()
        LogPageactivityCountByUser = FCAMiner.transitionDataMatrixConstruct_for_prediction_percentage(LogPageactivityCountByUser)
    # return LogPageactivityCountByUser
    ...
    for name, classifier in classifiers:
        ...
        scores_recall = cross_val_score(classifier, X, y, cv=10, scoring='recall_score')
        ...
        # result.update({name : [metricsResult, feature_imp, classifier,testingData, scores_f1, confusion_matrix, classification_report]})
    result.update({'data' : LogPageactivityCountByUser})
    return result

# def predictionWithPCAdata(data, algorithmList, exellentList, weakList, )
...
def predict_proba_all_algorithms_data_ready(PCAdata, excellentList, weakList,practice=[], mode = 'activity'):
    ...
    # classifiers = [('XGBoost',xgb),('Logistic Regression',lr),('Random Forest',rf),('Gradient Boosting',gb),
    #                    ('Decision Tree',dt),('SVM',svmL)]
    ...
    for name, classifier in classifiers:
        ...
        if name not in ['Logistic Regression','Linear Discriminant','Multi Layer Nets','SVM']:
            feature_imp = pd.Series(classifier.feature_importances_,index=X.columns).sort_values(ascending=False)
        else:
            feature_imp = 'None'
