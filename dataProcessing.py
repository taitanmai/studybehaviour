...
def get_event_log_by_file(m,u,d):
    corePath = 'anon/activity/' #'2020-06-29-einstein/activity/'
    path = corePath + m + '/'
    path += u + '/'
    path += d 
    """+ '-activity.log' """
...
def get_eventlogs_by_condition(module=[],user=[],date=[]):
    corePath = path =  'anon/activity/' #'2020-06-29-einstein/activity/'
    if len(module) == 0:
        path = corePath
...
def getUploadReportContent(m,u,d,file):
    corePath =  'anon/uploads/' #'2020-06-29-einstein/uploads/'
    path = corePath + m + '/'
    path += u + '/'
    path += d + '/'
    path += file
        ...
        for u in user:
            for d in date:
                ...
                if os.path.exists(path):
                    ...
                    for f in files:                        
                        if re.search(".report.(20|19)\d{2}-[0-1][0-9]-[0-3][0-9]-[0-2][0-9]_[0-6][0-9]_[0-6][0-9].json$",f):
                            print(f)
...
def getUploadReportContent_2019(m,u,d,file):
    corePath = '2020-06-29-einstein/uploads/' #'einstein-anon/anon/uploads/'
    path = corePath + d + '/'
    path += m + '/'
    path += u + '/'
    path += file
...
def extractUploadsData_2019(module = [],user=[],date=[]):
    corePath = '2020-06-29-einstein/uploads/' #'einstein-anon/anon/uploads/'                    
            ...
            for u in user:
                ...
                if os.path.exists(path):
                    ...
                    for f in files:                        
                        if re.search(".report.(20|19)\d{2}-[0-1][0-9]-[0-3][0-9]-[0-2][0-9]_[0-6][0-9]_[0-6][0-9].json$",f):
                            # print(f)
                            temp = getUploadReportContent_2019(m,u,d,f)
                            resultArray.append(temp)
    # print(resultArray)
    result = pd.DataFrame(resultArray,columns=columns)
    result['date'] = pd.to_datetime(result.date)
...
# result = extractUploadsData(module = ['ca117'])   
# result.to_csv("ca117_uploads.csv",index=False)    
...
# log1.to_csv("ca117_eventLog_nonfixed.csv",index=False) 
